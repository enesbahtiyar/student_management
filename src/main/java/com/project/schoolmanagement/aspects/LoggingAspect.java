package com.project.schoolmanagement.aspects;
...
public class LoggingAspect {
    ...
    @After("execution(* com.project.schoolmanagement.service.user.AdminService.*(..))")
    public void createLogForAdminServiceMethods(JoinPoint joinPoint) {
        logAuthentication(joinPoint);
    }

    @After("execution(* com.project.schoolmanagement.service.*.*.delete*(..))")
    public void createLogForDeleteOperations(JoinPoint joinPoint) {
        logAuthentication(joinPoint);
    }

    @AfterThrowing(pointcut = "execution(* com.project.schoolmanagement.service..*(..)) " +
            "|| execution(* com.project.schoolmanagement.repository..*(..))" +
            "|| execution(* com.project.schoolmanagement.contoller..*(..))" +
            "|| execution(* com.project.schoolmanagement.payload..*(..))", throwing = "e")
    public void logException(JoinPoint joinPoint, Throwable e) {
        log.error("Exception in method {}: {}", joinPoint.getSignature().getName(), e.getMessage());
    }

    @Around("execution(* com.project.schoolmanagement.service.*.*.*(..))")
